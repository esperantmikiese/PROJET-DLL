...
def verifier():
    ...
    if not aa.get() or aa.get() == "SELECT":
        ...
        a=1
    if not bb.get():
        txt.insert(INSERT,"Tapez le prénom \n")
        b=1
    if not cc.get():
        txt.insert(INSERT,"Tapez le nom de famille \n")
        c=1
    if not dd.get() or dd.get() == "SELECT":
        txt.insert(INSERT,"Selectionnez la classe correctement \n")
        d=1
    if not ee.get() or ee.get() == "SELECT":
        txt.insert(INSERT,"Selectionnez la  Section correctement \n")
        e=1
    if not ff.get() or ff.get() == "SELECT":
        txt.insert(INSERT,"Selectionnez le sexe correctement \n")
        f=1
    if not gg.get() :
        txt.insert(INSERT,"Tapez le nom du père \n")
        g=1
    if not hh.get() :
        txt.insert(INSERT,"Tapez le nom de la mère \n")
        h=1
    if not ii.get() or ii.get() == "SELECT":
        txt.insert(INSERT,"Tapez le numéro de mobile\n")
        i=1
    if not jj.get():
        txt.insert(INSERT,"Tapez l'adresse\n")
        j=1
